...
// set local variables
$connect = mysql_connect("213.168.251.122", "estoniac_hool", "1WhoLetDogs") or die('Could    not connect: ' . mysql_error());
$handle = fopen("downloaded.csv", "r");

// connect to mysql and select database or exit 
mysql_select_db("estoniac_hool", $connect);
...
while (($data = fgetcsv($handle, 1000, ';', '"')) !== false) {
	...
	if ($line = mysql_fetch_array($result, MYSQL_ASSOC)) {
		...
		// entry exists update
		$query = "UPDATE products SET price ='$price' , quantity = '$stock' WHERE original_number = '$Product_ID'";
		...
		echo " entry for ".$Product_ID." doesn't exist continue on next line";
	}
